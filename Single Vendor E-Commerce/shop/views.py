...
def product_list(request,category_slug=None):
    ...
    if rating_value:
        products=products.annotate(avg_rating=Avg('ratings__rating')).filter(avg_rating__gte=rating_value)
...
def product_detail(request,slug):
    ...
    return render(request,'shop/product_detail.html',{
        ...
        'can_review':purchased_item_exists,
...
def payment_failed(request,order_id):
    ...
    order.save()
    return render(request,'shop/payment_fail.html',{'order':order})
...
def profile(request):
    ...
    total_spent=sum(order.get_total_price() for order in orders if order.paid)
...
def order_detail(request, order_id):
    ...
    can_review_map = {
        item.product_id: OrderItem.objects.filter(
            order__user=request.user,
            order__paid=True,
            product=item.product,
        ).exists()
